...
def insert_row(email, name, password, phone, driving_license_number, D_O_B, license_plate_number, card_number, card_expiry, cvv, user_name=config.mysql_user, mysql_password=config.mysql_password, host_name=config.mysql_host, db_name=config.db_name, table_name=config.table_name):
    # Define our connection string
    conn = MySQLdb.connect(user=user_name, password=mysql_password, host=host_name, database=db_name)
    try:
        cursor = conn.cursor()
        cursor.execute("""insert into %s (email, name, password, phone,driving_license_number,D_O_B,license_plate_number, card_number, card_expiry, cvv) values ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d'); """ %(table_name, email, name, password, phone, driving_license_number, D_O_B, license_plate_number, card_number, card_expiry, cvv))
        conn.commit()
    finally:
        conn.close()
    print('Row inserted')
